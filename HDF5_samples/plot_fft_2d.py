"""
Example script to plot 2-dimensional Power Spectral Density (f-x) plot of Treble hdf5 data.
Converts velocity data to strainrate if required.
"""
...
def plot_2d(data, y, x, title=None, units=None, axis=None, cmap="inferno"):
    if axis is not None:
        plt.sca(axis)
    else:
...
freqs, psd = welch(data, fs=1/metadata["dt_computer"], axis=0, nperseg=len(data), noverlap=0)

plot_2d(10*np.log10(psd), freqs, x, title=hdf_file, units='PSD (dB)')

t_start = datetime.utcfromtimestamp(t[0])
plt.title(f"UTC {t_start}", loc="left", fontsize=10)

plt.gca().invert_yaxis()
plt.savefig("plot_fft_2d.png")
plt.show()