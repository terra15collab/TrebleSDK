...
if metadata['data_product'] in ["velocity", "deformation", "velocity_filtered", "deformation_filtered"]:
    # velocity plot
    ax1 = plt.subplot(211)
    plot_2d_heatmap(data, t, x, title=f"{metadata['data_product']}", ax=ax1)

    # converts to strain rate
    strainrate_data = convert_velocity_to_strainrate(data, metadata["dx"], gauge_length)
    x_corrected = correct_gauge_length_offset(x, gauge_length)

    # strain rate plot
    ax2 = plt.subplot(212, sharex=ax1)
    plot_2d_heatmap(strainrate_data, t, x_corrected, title=f"Strain Rate, Gauge length = {gauge_length}m", ax=ax2)

elif metadata["data_product"] == "strain_rate":
    # plots only strainrate data
    plot_2d_heatmap(data, t, x, title=f"Strain Rate, Gauge length = {metadata['gauge_length']}m", ax=plt.gca())

plt.tight_layout()
plt.savefig(f"plot_hdf5_raw.png")
plt.show()